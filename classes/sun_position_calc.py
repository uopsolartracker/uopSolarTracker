...
class sun_position(base)
	...
	def ElapsedJD (self, Year, Month, Day, Hours, Minutes, Seconds):
	...
	def EclipticCoordinates (self, ElapJulianDays):
	...
	def CelestialCoordinates(self, EclipLongitude, EclipObliquity):
		...
		if RightAscension < 0.0 :
			RightAscension= RightAscension + self.twopi;
	...
	def LocalCoordinates (self, ElapJulianDays, DecimalHours, RightAscension, Declination):
		GreenwichMeanSiderealTime = 6.6974243242 + 0.0657098283*ElapJulianDays + DecimalHours;
		LocalMeanSiderealTime = (GreenwichMeanSiderealTime*15 + self.dLongitude)*self.rad;
		HourAngle = LocalMeanSiderealTime-RightAscension;

		Latidude_rad =self.dLatitude*self.rad;
		...
		if Azimuth < 0.0:
			Azimuth = Azimuth + self.twopi;
		
		Azimuth = Azimuth/self.rad;
		
		return (ZenithAngle, Azimuth)

	## -- Calculating Parallax Correction--
	def ParallaxCorrection (self, ZenithAngle):
		Parallax = (self.dEarthMeanRadius/self.dAstronmicalUnit)*math.sin(ZenithAngle);
		ZenithAngle=(ZenithAngle + Parallax)/self.rad;
	...
	def excel_position(self, x_azimuth, y_zenith, Month, Day, Year):
