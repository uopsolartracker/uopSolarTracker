...
class weatherData(base):
	...
	def getTimeOfDay(self):
		...
		if timeFloat >= eightTenAM and timeFloat <= fourFiftyPM:
			self.topCoverCodes['dayTime'] = 1
			#print("DAY = ", topCoverCodes['dayTime'])
			return
		self.topCoverCodes['dayTime'] = 0
	...
	def fillTable(self, jsonStringForecast, j):
		...
		if forecastLocalTimeString[11:] == "08:00:00":		
			...
			### 2.1) Fill truth table inputs
			if self.idCodeDictionary[jsonStringForecast['list'][j]['weather'][0]['id']] == 'x':
				self.topCoverCodes["weatherEightAM"] = 1

			if self.idCodeDictionary[jsonStringForecast['list'][j+1]['weather'][0]['id']] == 'x':
				self.topCoverCodes["weatherElevenAM"] = 1

			if self.idCodeDictionary[jsonStringForecast['list'][j+2]['weather'][0]['id']] == 'x':
				self.topCoverCodes["weatherTwoPM"] = 1	

			if self.idCodeDictionary[jsonStringForecast['list'][j+3]['weather'][0]['id']] == 'x':
				self.topCoverCodes["weatherFivePM"] = 1
			...
			self.topCoverCodes["eightAMTimestamp"] = datetime.datetime.strptime(str(forecastLocalTimeString), "%Y-%m-%d %H:%M:%S").timestamp()	
			# 11AM
			forecastLocalTimeString = datetime.datetime.fromtimestamp(jsonStringForecast['list'][j+1]['dt'])
			self.topCoverCodes["elevenAMTimestamp"] = datetime.datetime.strptime(str(forecastLocalTimeString), "%Y-%m-%d %H:%M:%S").timestamp()
			# 2PM
			forecastLocalTimeString = datetime.datetime.fromtimestamp(jsonStringForecast['list'][j+2]['dt'])
			self.topCoverCodes["twoPMTimestamp"] = datetime.datetime.strptime(str(forecastLocalTimeString), "%Y-%m-%d %H:%M:%S").timestamp()	
			# 5PM				
			forecastLocalTimeString = datetime.datetime.fromtimestamp(jsonStringForecast['list'][j+3]['dt'])
			self.topCoverCodes["fivePMTimestamp"] = datetime.datetime.strptime(str(forecastLocalTimeString), "%Y-%m-%d %H:%M:%S").timestamp()

			print("TopCoverCodes = ",json.dumps(self.topCoverCodes, sort_keys=True, indent=4))
			print(" ")
	...
	#12 	#    1    |     1     |    0    |    0    || 	2:10pm - 4:50pm
		...
		# 0
		if self.topCoverCodes["weatherEightAM"] == 0 and self.topCoverCodes["weatherElevenAM"] == 0 and self.topCoverCodes["weatherTwoPM"] == 0 and self.topCoverCodes["weatherFivePM"] == 0:
			print("In process, #0")
			# If daytime, find when to start the gap time
			if self.topCoverCodes["dayTime"] == 1:
				gap_day_time(0)
			elif self.topCoverCodes["dayTime"] == 0:
				self.topCoverCodes["gapStart"] =  self.topCoverCodes["eightAMTimestamp"] + TENMINUTES
				self.topCoverCodes["gapEnd"] =  self.topCoverCodes["fivePMTimestamp"] - TENMINUTES

		# 1
		elif self.topCoverCodes["weatherEightAM"] == 0 and self.topCoverCodes["weatherElevenAM"] == 0 and self.topCoverCodes["weatherTwoPM"] == 0 and self.topCoverCodes["weatherFivePM"] == 1:
			print("In process, #1")
			if self.topCoverCodes["dayTime"] == 1:
				gap_day_time(1)
			elif self.topCoverCodes["dayTime"] == 0:		
				self.topCoverCodes["gapStart"] =  self.topCoverCodes["eightAMTimestamp"] + TENMINUTES
				self.topCoverCodes["gapEnd"] =  self.topCoverCodes["twoPMTimestamp"] - TENMINUTES
		
		# 2
		elif self.topCoverCodes["weatherEightAM"] == 0 and self.topCoverCodes["weatherElevenAM"] == 0 and self.topCoverCodes["weatherTwoPM"] == 1 and self.topCoverCodes["weatherFivePM"] == 0:
			print("In process, #2")
			if self.topCoverCodes["dayTime"] == 1:
				gap_day_time(2)
			elif self.topCoverCodes["dayTime"] == 0:
				self.topCoverCodes["gapStart"] =  self.topCoverCodes["eightAMTimestamp"] + TENMINUTES
				self.topCoverCodes["gapEnd"] =  self.topCoverCodes["elevenAMTimestamp"] - TENMINUTES

		# 3
		elif self.topCoverCodes["weatherEightAM"] == 0 and self.topCoverCodes["weatherElevenAM"] == 0 and self.topCoverCodes["weatherTwoPM"] == 1 and self.topCoverCodes["weatherFivePM"] == 1:
			print("In process, #3")
			if self.topCoverCodes["dayTime"] == 1:
				gap_day_time(3)
			elif self.topCoverCodes["dayTime"] == 0:		
		...
		# 4
		elif self.topCoverCodes["weatherEightAM"] == 0 and self.topCoverCodes["weatherElevenAM"] == 1 and self.topCoverCodes["weatherTwoPM"] == 0 and self.topCoverCodes["weatherFivePM"] == 0:
			print("In process, #4")
			if self.topCoverCodes["dayTime"] == 1:
				gap_day_time(4)
			elif self.topCoverCodes["dayTime"] == 0:		
				self.topCoverCodes["gapStart"] =  self.topCoverCodes["twoPMTimestamp"] + TENMINUTES
				self.topCoverCodes["gapEnd"] =  self.topCoverCodes["fivePMTimestamp"] - TENMINUTES

		# 8
		elif self.topCoverCodes["weatherEightAM"] == 1 and self.topCoverCodes["weatherElevenAM"] == 0 and self.topCoverCodes["weatherTwoPM"] == 0 and self.topCoverCodes["weatherFivePM"] == 0:
			print("In process, #8")
			if self.topCoverCodes["dayTime"] == 1:
				gap_day_time(8)
			elif self.topCoverCodes["dayTime"] == 0:		
				self.topCoverCodes["gapStart"] =  self.topCoverCodes["elevenAMTimestamp"] + TENMINUTES
				self.topCoverCodes["gapEnd"] =  self.topCoverCodes["fivePMTimestamp"] - TENMINUTES

		# 12
		elif self.topCoverCodes["weatherEightAM"] == 1 and self.topCoverCodes["weatherElevenAM"] == 1 and self.topCoverCodes["weatherTwoPM"] == 0 and self.topCoverCodes["weatherFivePM"] == 0:
			print("In process, #12")
			if self.topCoverCodes["dayTime"] == 1:
				gap_day_time(12)
			elif self.topCoverCodes["dayTime"] == 0:		
	...
	def gap_day_time(self, truthTableVal):
		...
		if truthTableVal == 0:
			if timefloat > eightAM and timefloat < elevenAM:
				...
				self.topCoverCodes["gapStart"] = 0
				self.topCoverCodes["gapEnd"] = 0
				self.topCoverCodes["topCurrentStatus"] = 0
	...
	def resetCodesAndTimes(self, r):
		...
		if r == 1:
			print("In reset, 1")
			self.topCoverCodes['weatherEightAM'] = 0
			self.topCoverCodes['eightAMTimestamp'] = 0.0
		elif r == 2:
			print("In reset, 2")
			self.topCoverCodes['weatherEightAM'] = 0
			self.topCoverCodes['weatherElevenAM'] = 0
			self.topCoverCodes['eightAMTimestamp'] = 0.0
			self.topCoverCodes['elevenAMTimestamp'] = 0.0
		elif r == 3:
			print("In reset, 3")
			self.topCoverCodes['weatherEightAM'] = 0
			self.topCoverCodes['weatherElevenAM'] = 0
			self.topCoverCodes['weatherTwoPM'] = 0
			self.topCoverCodes['eightAMTimestamp'] = 0.0
			self.topCoverCodes['elevenAMTimestamp'] = 0.0
			self.topCoverCodes['twoPMTimestamp'] = 0.0
		elif r == 4:
			print("In reset, 4")
			self.topCoverCodes['weatherEightAM'] = 0
			self.topCoverCodes['weatherElevenAM'] = 0
			self.topCoverCodes['weatherTwoPM'] = 0
			self.topCoverCodes['weatherFivePM'] = 0
			self.topCoverCodes['eightAMTimestamp'] = 0.0
			self.topCoverCodes['elevenAMTimestamp'] = 0.0
			self.topCoverCodes['twoPMTimestamp'] = 0.0
			self.topCoverCodes['fivePMTimestamp'] = 0.0
